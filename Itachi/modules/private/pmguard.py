...
from Itachi.database.pmpermitdb import get_approved_users, pm_guard
import Itachi.database.pmpermitdb as Itachi
...
async def pmguard(client, message):
    ...
    if not arg:
        ...
        return
    await Itachi.set_limit(int(arg))
...
async def setpmmsg(client, message):
    ...
    if not arg:
        ...
        return
    if arg == "default":
        await Itachi.set_block_message(Itachi.BLOCKED)
        await message.edit("**Block message set to default**.")
        return
    await Itachi.set_block_message(f"`{arg}`")
...
async def allow(client, message):
    ...
    pmpermit, pm_message, limit, block_message = await Itachi.get_pm_settings()
    await Itachi.allow_user(chat_id)
...
async def deny(client, message):
    chat_id = message.chat.id
    await Itachi.deny_user(chat_id)
